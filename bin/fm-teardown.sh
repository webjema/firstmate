...
# The local worktree can legitimately LAG the merged PR: a crew's own review-and-fix
# rounds and any CI-fix rounds push extra commits to the PR branch, and those commits
# need not exist locally. So the check compares against the PR head, not just what the
# worktree happens to hold.
# The PR itself is resolved from the task's recorded pr= when present, or - when
# no pr= was ever recorded (e.g. a yolo-authorized merge on a repo with no PR CI,
# where the usual PR-ready fm-pr-check.sh trigger never fires) - by looking
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.status" "$sub_state/$child_id.turn-ended" "$sub_state/$child_id.check.sh" "$sub_state/$child_id.ci-seen" "$sub_state/$child_id.meta" "$sub_state/$child_id.pi-ext.ts" "$sub_state/$child_id.grok-turnend-token" "$sub_state/.turnend-seen-$child_id" "$sub_state/.wt-size-$child_id"
  done
}
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.check.sh" "$STATE/$ID.ci-seen" "$STATE/$ID.meta" "$STATE/$ID.pi-ext.ts" "$STATE/$ID.grok-turnend-token" "$STATE/.turnend-seen-$ID" "$STATE/.wt-size-$ID"
