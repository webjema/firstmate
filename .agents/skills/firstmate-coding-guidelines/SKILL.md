...
- Run `bin/fm-lint.sh` before treating a script change as done; it is the single owner of the lint definition (file set, config, and pinned shellcheck version) that CI and the project's own quality hooks both invoke, and it refuses to run under any other shellcheck version.
