...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   ...
   - **Harness WITH a native in-pane tracked-background tool** (e.g. claude's
     ...
     The launcher still owns lifecycle state and records the no-terminal mode, while the daemon inherits and auto-discovers the user pane.
     If the native launch fails, run `bin/fm-afk-launch.sh stop` to roll back the prepared lifecycle.
     Do not wrap it in `nohup ... &` (Codex can reap fire-and-forget shell children after a tool call returns).
   - **Harness WITHOUT one** (e.g. pi): run `bin/fm-afk-launch.sh start`. It is
     ...
     terminal (a detached tmux session), records its exact id, and passes the
     user pane in as `FM_SUPERVISOR_TARGET` so the daemon injects into the user,
     not its own new pane. **Never manufacture a terminal by splitting the
     user's active pane**: a split co-tenants the window and visibly shrinks the
     user's pane.
...
- A message **without** the sentinel marker and **not** starting with `/afk` -> the user is back.
...
injection, dispatched through `bin/fm-backend.sh` for the supervisor's own tmux backend:

- **`pane_is_busy`** - the harness shows a busy footer (agent mid-turn), shared with `fm-send.sh` via `bin/fm-tmux-lib.sh`.
- **Composer-state guard** - `inject_msg` reads the full `empty`/`pending`/`unknown` verdict from `fm_backend_composer_state` and injects only when it is affirmatively `empty`.
  ...
  The shared `bin/fm-composer-lib.sh` owns the content decision after the backend captures and structurally identifies its own composer row.
  It preserves idle bordered composers such as claude's `│ > … │` and bare agent glyphs as empty, but a bare shell glyph is unknown unless inside a genuine bordered composer box.
...
In afk mode the composer guard is belt-and-suspenders (no human is typing), but it protects against the race window between the user returning and their message landing, a dead shell, and the daemon's own previous injection sitting unsent.

**Max-defer escape (the daemon must never silently wedge).**
If anything stays buffered past `FM_MAX_DEFER_SECS` (default 300), the daemon runs a two-step escape.
...
So a guard false-positive becomes a delivered digest, and a genuinely unreachable pane becomes a visible stall, never an unbounded silent no-op.

## Submit model

The digest is typed **once** (`send-keys -l` on tmux, a literal non-submitting
send), then submitted with Enter and **verified** through the backend's submit primitive.
...
- **Single-line digest** - embedded newlines are collapsed to a literal
  ...
  harness.
- **Composer guard** - before injecting, the daemon checks `pane_is_busy` and reads `fm_backend_composer_state`, injecting only on an affirmatively `empty` composer, per Busy-guard and composer guard above.
...
- **Auto-discovered supervisor pane** - the daemon resolves its own TARGET
  ...
  is detectable. Other runtime backends, including zellij, orca, and cmux, are
  not yet supported as supervisor backends; the daemon refuses loudly at startup
