...
3. **Dispatch one direct-fix ship crew scoped to the slice.**
   ...
   - **What to fix.** Duplication (merge near-identical functions into one clear function, or split an overloaded one); maintainability and testability smells; dead code; and comment hygiene - delete comments that point at throwaway work-in-progress docs or stale TODOs, and leave only compact comments that explain *why*, where a reader a year from now needs them.
   - **The bar.** Every change moves with the quality direction above, and the test suite stays green - the tests are the regression net for a refactor.
...
The comment-hygiene bar this skill enforces - no comments pointing at throwaway work-in-progress docs, compact and why-focused only - is a standing quality principle, not a one-pass whim.
