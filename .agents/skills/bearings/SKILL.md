...
1. **Gather live fleet state with one deterministic command.**
   ...
   When the user asks to include PRs, use the command's live-PR opt-in; otherwise keep the default local-only read.
...
2. **Compose the detailed report file around the four-section spine, adding the richer detail the chat leaves out.**
   ...
   - **Title** - `# Bearings - <day> <YYYY-MM-DD>` (use "Morning status" only when the user specifically asks for a morning brief), followed by two or three sentences framing where things stand.
...
1. **Captain's Call** - ONLY items that need the user's own action now: a decision to make, a PR to approve or merge, a credential or login to provide, or a blocker only the user can clear.
...
4. **Charted Next** - queued or gated work waiting on the fleet or a date, never on the user.
...
- This report is a private, user-facing internal artifact that lives in gitignored `data/`, so unlike normal user chat it MAY reference task ids, PR URLs, and repo names - the user works with these directly and needs them to resume; keep it organized and scannable, not a raw dump.
...
If the state you read suggests an action - a PR ready to merge, a queued item whose gate has arrived, a needs-decision finding - name it in its section (a user action under "Captain's Call", queued or gated work under "Charted Next") and let the user decide, rather than taking the action from inside this skill.
