...
FM_DISK_CDK_AGE_DAYS=3          # age past which a cdk.out* synth staging dir is dead. These are output, not a cache - the next synth rebuilds them - so the age window is the whole safety model; raise it if you inspect cloud assemblies after the run
FM_DISK_TMP_ROOT=/tmp           # where the cdk sweep looks for staging dirs
