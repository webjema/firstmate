...
Beyond the durable `state/.subsuper-inject-wedged` marker and the tmux status-line flash, it attempts a configured active alert that can reach the user even when the pane and its status-line are unreadable.
...
The primary firstmate session is user-launched, so firstmate cannot inject its own env; to hold the primary to the same ceiling, launch it with those two env vars set (for example `CLAUDE_CODE_AUTO_COMPACT_WINDOW=200000 CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=90 claude ...`), or put them in the `env` block of a personal, gitignored `.claude/settings.local.json`.
Firstmate's recovery is designed for exactly this: because its conversation is a cache and all truth lives in `state/`, `data/`, and tmux, a compaction or restart is a non-event.
...
Fleet-local operational facts and gotchas live locally in `data/learnings.md`; it is gitignored and printed right after `data/user.md` in the session-start context digest.
The file is created lazily on first learning and follows the same dated, evidence-backed, curated style as `data/user.md`: inspect the current file first, then rewrite or prune stale entries instead of appending forever.
...
Sharing turns three previously private things into contended ones, all owned by [`bin/fm-peer-lib.sh`](../bin/fm-peer-lib.sh):
a per-clone write lock so two instances syncing one clone take turns instead of one going stale,
a live-peer session registry so `bin/fm-update.sh` refuses to swap `bin/` beneath a peer that is mid-turn,
and a per-task temp root named from the home so two instances running a task with the same id do not share one directory or delete each other's.
The worktree pool is shared already: treehouse keys a pool by the clone's basename and its origin URL, so two clones of one upstream resolve to one pool whatever their paths, and `bin/fm-pool-lib.sh` derives the warm lock the same way so both instances contend for the same lock.

`FM_CLONE_LOCK_WAIT_SECS` bounds how long an instance waits for the per-clone write lock, defaulting to 300; on expiry the caller reports and skips rather than hanging.
...
FM_DISK_PATH=/tmp               # filesystem bin/fm-disk-guard.sh measures; the sweeps always act on their own allowlisted roots regardless
FM_DISK_WARN_FREE_GB=20         # free space at or below which the guard's warn tier fires. Paired with a percentage below because neither survives both box sizes alone: a 4T disk at 92% has 320G free and needs nothing
FM_DISK_CRITICAL_FREE_GB=8      # ... and the critical tier
FM_DISK_WARN_USED_PCT=85        # used percentage at or above which the warn tier fires; whichever of the two signals reads worse picks the tier
FM_DISK_CRITICAL_USED_PCT=92    # ... and the critical tier
FM_DISK_CACHES='npm tmpbuild playwright'  # which cache families the critical tier may clear; empty disables all of them. Complements FM_POOL_DISK_BUDGET_GB, which is preventive and per-pool, where this is reactive and whole-disk
FM_DISK_GUARD_INTERVAL=900      # seconds between watcher disk checks. Flat, unlike the backing-off heartbeat: a disk fills fastest exactly when the fleet is busiest
FM_DISK_GUARD_TIMEOUT=120       # ceiling on one guard run from the watcher; the supervision loop must never be stalled by a janitor
...
FM_ARM_STANDBY_POLL=1   # seconds between checks while a duplicate arm is parked waiting for the arm that owns supervision to end
FM_ARM_STANDBY_SETTLE=0.5   # seconds a parked arm waits after taking the arming marker before trusting it won the promotion race
FM_ARMING_GRACE=30      # seconds an UNVERIFIABLE state/.watch.arming marker (a SIGKILLed arm's orphan) is still tolerated by the guards; a live identity-matched arm counts regardless of marker age
FM_ARM_RELAUNCH_MAX=8   # information-free watcher deaths fm-watch-arm replaces in place before reporting FAILED instead of relaunching again
FM_ARM_CHURN_WINDOW=600 # seconds after which the quiet-exit count resets, so slow churn over hours never accumulates into a false alarm
FM_ARM_BACKOFF_MAX=30   # cap on the exponential backoff between quiet-death relaunches
FM_ARM_LOG_MAX=200      # lines kept in state/.watch-arm.log, the per-relaunch churn record
...
FM_PENDING_FROZEN_SECS=600         # seconds a pending composer must stay byte-identical under continuous observation before the force flush drains it; 0 restores the unbounded refusal
FM_PENDING_SAMPLE_GAP_MAX=120      # longest gap between two composer observations that still counts as continuous; a longer gap restarts the freeze window
FM_AFK_PREFLIGHT=1                 # away-mode entry proves the supervisor composer reads empty before starting; 0 skips the check and leaves the wake path unverified
