...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review, and it has exactly two sources: the crew's recorded backend endpoint and its status log.
A busy endpoint signature is live, positive evidence and outranks the log, so a crew that has resumed past an answered `needs-decision` reads as working instead of re-escalating a settled decision.
Only an idle endpoint falls back to the status log's last line, and only when its verb maps to a recognized state; a dead or unreadable endpoint reports unknown instead of trusting a stale log.
...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it starts the watcher, verifies it is genuinely alive with a fresh liveness beacon, and prints honest status lines (`started` / `attached` / `standby` / `relaunching` / `wakes queued` / restart-only `healthy` / `FAILED`, the last exiting non-zero).
The watcher runs in the home's own detached tmux session, not as a child of the arm, because a harness reaps a background task by walking its ppid descendants and `setsid` does not change a ppid - so a watcher forked from the arm died on every reap with supervision silently off.
[incidents/watcher-harness-reap.md](incidents/watcher-harness-reap.md) holds that measurement; a `setsid` child remains the fallback where no tmux server can be reached.
Ownership is recorded in `state/.watch.owner-arm` rather than read from the watcher's parent, which is now the tmux server for every watcher and would read as unowned.
On a notify-on-exit harness the arm's own exit IS the wake, so it exits ONLY on information: a watcher wake reason, wake records left in the durable queue, or one bounded `FAILED`.
A watcher that ends with nothing to report is relaunched in place, with exponential backoff and a churn budget (`FM_ARM_RELAUNCH_MAX` quiet exits inside `FM_ARM_CHURN_WINDOW`) that turns a watcher which will not stay up into a single loud failure instead of a relaunch loop; every relaunch is recorded in `state/.watch-arm.log`.
One arm per home: a second arm becomes a silent `standby` that neither attaches to the live cycle nor exits, and takes supervision over only when the arm it parked on is gone.
Both halves are load-bearing - attaching means two harness notifications per wake, and exiting means a notification carrying no news - and each one was a live incident.
[incidents/watch-arm-notification-storm.md](incidents/watch-arm-notification-storm.md) records the storm that established these rules.
...
`bin/fm-backend.sh` centralizes backend selection, `state/<id>.meta` helpers, selector resolution, and operation dispatch; `bin/backends/tmux.sh` is the one verified adapter ([`docs/tmux-backend.md`](tmux-backend.md)), and tmux is the only backend.
New spawns always resolve to tmux, and unknown backend names fail loudly.
For compatibility, default tmux tasks do not write `backend=tmux`; every reader treats a missing `backend=` field as `tmux`.
`fm-watch.sh` polls each window for a busy state: tmux has no native primitive and always reports unknown, so stale detection uses the original pane-tail regex.
That poll loop is the default event source for a backend with no native push events, so this stays an extraction of the abstraction rather than a watcher rewrite.
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux has a verified classifier for secondmate recovery.
...
Ship tasks change projects and ship by project mode (`PR` or `local-only`); scout tasks investigate, plan, reproduce bugs, or audit, then leave a report at `data/<id>/report.md` and never push.
...
`fm-home-seed.sh` provisions the isolated home, clones the listed PR-based projects into it, copies the charter to `data/charter.md`, and `fm-spawn.sh --secondmate` launches it through the same session-provider and status-file path as any direct report.
...
Fleet state lives in each task's session-provider backend (tmux), status event logs, local markdown under `data/` including `data/user.md` and `data/learnings.md`, and persistent secondmate homes.
