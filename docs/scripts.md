...
| `fm-graph-reindex.sh`    | Best-effort refresh of one already-indexed project's codebase-memory knowledge graph |
| `fm-graph-lib.sh`        | Single owner of how firstmate talks to the codebase-memory knowledge graph            |
...
| `fm-wake-kind-lib.sh`    | Single owner of the wake-kind vocabulary and the "is this line a wake?" predicates   |
