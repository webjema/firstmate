...
# The arm fixture - the scripted stand-in watcher and the two ways to run an arm
# against it - is shared with tests/fm-watch-arm-hosting.test.sh. This file is about
# what the arm may WAKE firstmate for, so it pins hosting to a plain child: these
# cases have no private tmux socket, and `auto` would put watcher sessions on the
# HOST's tmux server.
ARM_HOST=child
...
test_watcher_survives_a_process_group_reap() {
  ...
  set_mode "$dir" hold
  # This case and the setsid one below build their own arm invocation instead of
  # calling run_arm_bg, so each has to pin FM_WATCH_HOST itself. Both are about the
  # forked-child host specifically, and an arm left on `auto` here would host its
  # watcher on the HOST's tmux server - testing nothing and littering a live box.
  cat > "$launcher" <<SH
#!/usr/bin/env bash
...
SH
  ...
  ( exec env PATH="$dir/fakebin:$PATH" FM_ROOT_OVERRIDE="$dir" FM_HOME="$dir" \
      FM_STATE_OVERRIDE="$dir/state" FM_WATCH_HOST=child \
      FM_ARM_BACKOFF_MAX=1 FM_ARM_CONFIRM_TIMEOUT=6 \
